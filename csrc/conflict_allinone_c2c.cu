...
int main(int argc, char** argv) {
    ...
    for (int cp = 0; cp < root.size(); ++ cp) {
        ...
        create_comm_group_from_pattern(pp, root[cp], true);
        ...
        for (int i = SIZEIDX_START; i < SIZEIDX_END; ++ i) {
            ...
            if (pp->BACKEND.compare("MPI") == 0) {
                send_buf[pp->rank] = (int*)malloc(SIZE * sizeof(int));
                recv_buf[pp->rank] = (int*)malloc(SIZE * sizeof(int));
            }

            // WARMUP
            for (int _ = 0; _ < WARMUP; ++ _) {
                XXX_comm(pp, root[cp], send_buf, recv_buf, SIZE, pp->streams, pp->rank, pp->cur_comm, pp->mpi_requests);
                barrier(pp->BACKEND, pp->N_GPUs, pp->cur_mpi_comm, true);
            }
            barrier(pp->BACKEND, pp->N_GPUs, MPI_COMM_WORLD, true);
            ...
            if (pp->BACKEND.compare("MPI") == 0) {
                free(recv_buf[pp->rank]);
                free(send_buf[pp->rank]);
            }
